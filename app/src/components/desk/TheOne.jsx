...
export function TheOne({ task, onToggle, onToggleStep }) {
  return (
    <div class="bg-surface border border-border border-l-[3px] border-l-amber rounded-xl p-6 mb-2">
      ...
      <div class="flex items-start gap-3.5">
        ...
        <div class="flex-1">
          <div class={cn("text-[1.05rem] font-semibold mb-0.5", task.done && "line-through text-text-dim")}>
            {task.title}
          </div>
          ...
          {task.microSteps?.length > 0 && (
            <div class="mt-3.5 pt-3.5 border-t border-border">
              ...
              {task.microSteps.map((step) => (
                <div key={step.id} class="flex items-center gap-2.5 py-1.5">
                  <Checkbox
                    checked={step.done}
